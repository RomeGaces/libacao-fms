  ...
  <VDialog v-model="isOpen" max-width="500" transition="dialog-bottom-transition">
    <VCard class="custom-modal-shadow rounded-lg pa-5">
      ...
      <VCardSubtitle class="text-body-2 text-grey-darken-1 mb-5 description-text">
        Are you sure you want to proceed OBR
        <strong>{{ obr?.obr_no }}</strong> to the next process step?
        <span v-if="nextStage">
          This will move it from <strong>{{ currentStage }}</strong> to <strong>{{ nextStage }}</strong>.
        </span>
        <span v-else>
