        ...
        <div class="pds-form">
            <nav class="pds-nav">
                <ul>
                    ...
                    }" @click="handleTabNavigation(tab.key)">
                        {{ tab.label }}
            ...
            <Form ref="pdsFormRef" :active-tab="activeTab" v-model:formData="formData" :errors="errors" />
        </div>

        <template #footer>
            <Button variant="secondary" size="md" @click="close">Cancel</Button>

            <Button variant="primary" size="md" @click="onNextTab">
...
// Define props and setup remains the same...
// ... (props, pdsFormRef, showModal, activeTab, emit, tabs, etc.)

// --- (Existing setup and data definition) ---
...
const { handleSubmit, validate, validateAllTabs, formData, errors } = usePdsValidation(
    activeTab,
    props.formData || {}
);
...
// ... (buildPayload function remains the same) ...
function buildPayload(f) {
    // (Existing buildPayload implementation)
    // ...
    const payload = {
        // Personal fields
        ...
        plantilla_id: f.plantilla_id || null,
    };
...
// ... (rest of the code before onTabClick) ...
...
// -------------------------------------------------------------
// 💡 CRITICAL FIX: Update tab navigation handler
// -------------------------------------------------------------

async function handleTabNavigation(newTabKey) {
    const newTabIndex = tabs.findIndex(tab => tab.key === newTabKey);
    const currentTab = activeTab.value;
    const currentTabIndex = tabs.findIndex(tab => tab.key === currentTab);

    // 1. If clicking the current tab, do nothing.
    if (newTabKey === currentTab) return;

    // 2. If the user clicks a PREVIOUS tab, allow it (for review).
    if (newTabIndex < currentTabIndex) {
        activeTab.value = newTabKey;
        return;
    }

    // 3. If the user clicks the NEXT or a FUTURE tab, validate the CURRENT tab first.
    const { valid } = await validate();

    if (!valid) {
        // If current tab is invalid, mark it and block navigation.
        tabStatus.value[currentTab] = 'warning';
        notificationService.error(`Please complete the ${tabs[currentTabIndex].label} section before proceeding.`);
        return;
    }

    // If validation passed for the current tab, mark it valid.
    tabStatus.value[currentTab] = 'valid';

    // 4. Now, enforce chronological order (no skipping).
    // The user can only move to the IMMEDIATE next tab.
    if (newTabIndex === currentTabIndex + 1) {
        activeTab.value = newTabKey;
    } else if (newTabIndex > currentTabIndex + 1) {
        notificationService.warning("Please proceed to the next tab sequentially.");
        // Stay on the current tab
    }
}

// -------------------------------------------------------------
// 💡 onNextTab function is now simplified to use the new handler
// -------------------------------------------------------------
...
async function onNextTab() {
    ...
    if (nextTabIndex < tabs.length) {
        nextTabKey = tabs[nextTabIndex].key;
    }

    // 1. If we are NOT on the last tab, use the chronological handler to move forward.
    if (nextTabKey) {
        // If valid, handleTabNavigation will move to the next tab. If invalid, it will stay.
        await handleTabNavigation(nextTabKey);
        return;
    }

    // 2. If we ARE on the last tab, perform final validation/submission check.

    // Validate current (last) tab fields first
    const { valid } = await validate();

    if (!valid) {
        tabStatus.value[activeTab.value] = 'warning';
        notificationService.error("Please complete the required fields on this tab.");
        return;
    }
    ...
    // Update tabStatus for all tabs based on global validation result
    tabs.forEach(tab => {
        if (invalidTabs.includes(tab.key)) {
            tabStatus.value[tab.key] = 'warning';
        }
        else if (tab.key !== activeTab.value) {
            tabStatus.value[tab.key] = 'valid';
        }
    });
    ...
    } else if (invalidTabs.length > 0) {
        // Not all valid, navigate to the first invalid tab
        notificationService.error("Please complete all the required information before submission.");
        const firstInvalidTab = tabs.find(tab => invalidTabs.includes(tab.key));
        if (firstInvalidTab) activeTab.value = firstInvalidTab.key;
    }
};
...
// ... (watch, handleSave, open, close, defineExpose remain the same) ...

watch(
  () => props.formData,
  (newVal) => {
      if (newVal) {
          employeeId.value = newVal.id || null
...
async function handleSave(values) {
    ...
    try {
        const payload = buildPayload(values || formData.value || {});
        console.log("Payload being sent:", payload);
        if (employeeId.value) {
            await employeeService.update(employeeId.value, payload);
        } else {
            await employeeService.create(payload);
        }


        notificationService.success("Employee saved successfully!");
        ...
        notificationService.error("Failed to save employee.", errorMsg);
    }
}
...
function open() {
    ...
    Object.keys(tabStatus.value).forEach(key => tabStatus.value[key] = '');
}
function close() {
    showModal.value = false;
}
